...
import type { Record, SendOptions } from './types.js';
import { EXPIRATION_TIME } from './constants.js';
import { pocketBaseInsert } from './sorting.js';
...
export function createCollectionStore<T extends Record[]>(
	...
	function handleCreate(value: T[number]) {
	...
	function handleUpdate(value: T[number]) {
	...
	} as Writable<T> & { data: { create: any; update: any; delete: any } };
}

export function createItemStore<T extends Record>(pb: PocketBase, initialValue: T) {
